...
function getRecipesFromStorage() {
  ...
  let currRecipe = localStorage.getItem('recipes');
  if(!currRecipe)
    return [];
  return JSON.parse(currRecipe);
}
/**
...
function saveRecipesToStorage(recipes) {
  ...
  //            header. It is possible in only a single line, but should
  //            be no more than a few lines.
  let recipStr = JSON.stringify(recipes);
  localStorage.setItem('recipes', recipStr);
}
/**
...
function initFormHandler() {
  ...
  formElement.addEventListener('submit', (event)=>{
    ...
    let rcElement = document.createElement('recipe-card');
    // B7. TODO - Add the recipeObject data to <recipe-card> using element.data
    rcElement.data = recipeObject;
    // B8. TODO - Append this new <recipe-card> to <main>
    main.appendChild(rcElement);
